                    ...
                    GeneratedTrampoline::Utf8CopyTranscoder { from, to } => {
                        ...
                        Ok(Extern::Func(wasm_runtime_layer::Func::new(
                            ...
                            move |mut ctx, args, results| {
                                eprintln!(
                                    "transcode-copy-utf8-{from}-{to}({args:?}, {results:?}) enter"
                                );
                                let (from_ptr, len, to_ptr) = match args {
                                    [wasm_runtime_layer::Value::I32(from_ptr), wasm_runtime_layer::Value::I32(len), wasm_runtime_layer::Value::I32(to_ptr)] => {
                                        (from_ptr, len, to_ptr)
                                    }
                                    args => {
                                        bail!(
                            ...
                            results.is_empty(),
                            "transcode-copy-utf8-{}-{}(from-ptr: i32, to-ptr: i32, len: i32) \
                            call expects unexpected results {:?}",
                            from,
                            to,
                            results
                        );
                                ...
                                eprintln!(
                                    "transcode-copy-utf8-{from}-{to}({args:?}, {results:?}) exit"
                                );
                                Ok(())
                            },
                        )))
                        ...
                        let func = wasm_runtime_layer::Func::new(
                            ...
                            move |_ctx, args, results| {
                                ...
                                let (handle, from_rid, to_rid) = match args {
                                    [wasm_runtime_layer::Value::I32(handle), wasm_runtime_layer::Value::I32(from_rid), wasm_runtime_layer::Value::I32(to_rid)] => {
                                        (handle, from_rid, to_rid)
                                    }
                                    args => bail!(
                                "resource-transfer-own(handle: i32, from-rid: i32, to-rid: i32)\
                                 called with unexpected args {:?}",
                                args
                            ),
                                };
